...
import { _decorator, Color, Component, Font, instantiate, Prefab, EventTouch, Game, game, Graphics, input, Input, Label, Node, ResolutionPolicy, screen, Sprite, SpriteFrame, sys, Texture2D, UITransform, Vec3, view } from 'cc';
import { FIXTURES, SOURCE_SHA } from '../ui-greybox/Fixtures';
...
type Panel=''|'pause'|'win'|'lose'|'ad'|'error'|'restart'|'exit'|'settings'|'entry'|'share';

/** Deliberately isolated UI harness: no GameSession, SaveService, inventory or real ads. */
@ccclass('MMUIPreview')
export class MMUIPreview extends Component {
  ...
  private resumes=0;
  private selected='';
  private notice='UI-05 组件预览 · 静态玩法';
  private redraw=true;
  private cssScale=1;
  ...
  protected override onDestroy():void {
    ...
    if(typeof window!=='undefined') delete (window as unknown as Record<string,unknown>).__UI_PREVIEW__;
  }
  ...
  private nodeAt(parent:Node,name:string,x:number,y:number,w:number,h:number,prefab?:Prefab|null):Node {
  ...
  private button(parent:Node,name:string,label:string,r:Rect,action:()=>void,primary=false,state:ButtonState='normal'):void {
  ...
  private cat(p:Point,r:number,breed:number,name='Cat'+breed):void {
  ...
  private track():void {
    const f=FIXTURES[this.index],n=this.nodeAt(this.stage,'Track',0,0,0,0),g=n.getComponent(Graphics)??n.addComponent(Graphics);g.clear();
  ...
  private dialog():void {
    ...
    const titles:Record<Exclude<Panel,''>,string>={pause:'歇一会儿喵',win:'猫猫到家啦！',lose:'差一点到家',ad:'准备继续回窝',error:'暂时没有可用视频',restart:'重新开始这一关？',exit:'返回首页？',settings:'设置',entry:this.entryTitle,share:'分享回窝时刻'};
    ...
    if(this.panel==='pause'){
      ...
      b('next',this.index===6?'回到关卡':'下一关',650,()=>{this.index=(this.index+1)%FIXTURES.length;this.panel='';},true);
  ...
  private render():void {
    ...
    this.text(this.stage,'HarnessCaption','UI-05 · 可复用组件预览',375,26,700,32,22,C.muted);
    if(this.home){
    ...
    const controls:[string,string,()=>void][]=[['case','换关',()=>{this.cancelTouch();this.index=(this.index+1)%7;this.panel='';this.home=false;this.resumes=0;}],['safe','安全区',()=>{this.cancelTouch();this.notched=!this.notched;}],['states','状态',()=>{this.stateSample=(this.stateSample+1)%3;this.home=true;this.panel='';}],['win','胜利',()=>{this.home=false;this.show('win');}],['lose','复活',()=>{this.home=false;this.show('lose');}],['home','首页',()=>{this.cancelTouch();this.home=true;this.panel='';}]];
    if(this.panel==='ad')controls.splice(0,3,['adSuccess','完成',()=>{this.resumes++;this.show('');}],['adCancel','取消',()=>this.show('lose')],['adError','失败',()=>this.show('error')]);
    ...
    if(typeof window!=='undefined')(window as unknown as Record<string,unknown>).__UI_PREVIEW__={
      ...
      viewport:{width:w,height:h},engineViewport:view.getViewportRect(),visibleOrigin:view.getVisibleOrigin(),canvasWorld:this.node.worldPosition.clone(),safe,toolbarHeight,fontLoaded:!!this.uiFont,fit:this.fitted,next:PREVIEWS[this.index],topGap:e.top-HUD_BOTTOM,bottomGap:TOOL_TOP-e.bottom,
