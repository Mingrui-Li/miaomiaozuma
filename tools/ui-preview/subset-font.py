"""Build the licensed, renamed UI-05 font from the preserved upstream file."""
...
corpus=''.join(p.read_text() for folder in ['assets/ui-preview','assets/game/scripts/ui','assets/game/prefabs/ui'] for p in (root/folder).rglob('*') if p.suffix in ['.ts','.prefab'])
...
for record in font['name'].names:
    names={1:'MMHW UI Trial',2:'Regular',3:'MMHWUITrial-UI05',4:'MMHW UI Trial',6:'MMHWUITrial-Regular',16:'MMHW UI Trial',17:'Regular'}
    if record.nameID in names:record.string=names[record.nameID].encode(record.getEncoding())
dest=root/'assets/game/fonts/mmhw_ui_trial.ttf';dest.parent.mkdir(parents=True,exist_ok=True);font.save(dest)
...
report={'source':'https://github.com/notofonts/noto-cjk/blob/main/Sans/Variable/TTF/Subset/NotoSansSC-VF.ttf','license':'SIL OFL 1.1','version':version,'weight':500,'family':'MMHW UI Trial','sourceSHA256':hashlib.sha256(source.read_bytes()).hexdigest(),'subsetSHA256':hashlib.sha256(dest.read_bytes()).hexdigest(),'bytes':dest.stat().st_size,'characters':''.join(sorted(chars)),'missing':[],'scope':'UI-05 corpus only; regenerate coverage for later UI text'}
(root/'design/ui/ui05/font.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n')
