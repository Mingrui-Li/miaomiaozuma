...
async (page) => {
  ...
  await page.locator('#GameCanvas').screenshot({path:'output/playwright/ui04/pause.png'});
  ...
  for(let i=0;i<7;i++){
    ...
    await page.locator('#GameCanvas').screenshot({path:`output/playwright/ui04/l${String(s.level).padStart(3,'0')}.png`});
  ...
  await page.locator('#GameCanvas').screenshot({path:'output/playwright/ui04/home.png'});
  await click('start');await click('debug-win');await page.locator('#GameCanvas').screenshot({path:'output/playwright/ui04/win.png'});
  await click('next');
  const sizes=[];
  for(const [width,height] of [[360,640],[390,844],[430,932]]){
    ...
    await page.locator('#GameCanvas').screenshot({path:`output/playwright/ui04/ad-safe-${width}.png`});
    await click('freeRestart');
    sizes.push({width,height,canvas:r,fit:s.fit,safe:s.safe});
    await page.locator('#GameCanvas').screenshot({path:`output/playwright/ui04/l087-safe-${width}.png`});
  }
