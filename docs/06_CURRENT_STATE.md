...
最后核对：2026-09-15，总控 `/root`（UI-08本地发布准备交付）。这是最新快照；不要继续执行旧对话中的批量生图。

### D025 · 阶段口径修正

UI-08设计阶段已完成。所有者明确正式玩法、平台接入、美术及安卓验收在后续代码开发阶段执行，不是UI-08阻断。检查器改为`uiStagePassed=true` / `UI_STAGE_COMPLETE`，整游戏发布标记为`DEFERRED_TO_CODE_DEVELOPMENT`，移除旧固定`releaseReady=false`。原截图/构建/实测数据保持不变；默认UI检查及8项测试通过。仍有UI-08未提交文件，本窗口没有执行Git提交或推送。

## UI-08 本地发布准备 · 2026-09-15

- D023授权UI-08；新增 `tools/ui-release/`、`design/ui/ui08/`、332张精简构建截图及 [17发布准备报告](17_UI_RELEASE_READINESS.md)。只在临时工程关闭debug并裁剪未用引擎模块，没有重画UI或改原工程设置。页面版2,584,075 B、百关审查版6,723,256 B，分别减少78.19%/56.87%；不是抖音包体通过声明。
- 两次Creator实际构建退出36；完整页面点击/三尺寸及百关三尺寸×三状态900检查点通过，8份浏览器回执diagnostics为空。332张截图与UI-06/07对应原PNG哈希全部一致；12纹理及场景字体逐字节追溯，许可证随包。原200个asset/settings/profiles/几何文件与本次开始快照一致。
- 每版3次禁用HTTP缓存导航、40次弹窗/5秒静态rAF采样；页面启动约5.35/3.18/3.21秒、百关约3.23/3.21/3.05秒，页面首样本超过五秒目标。近似JS堆增长后各补120次回收后诊断，保留节点95/57、GC后净增长约1.20/0.66 MB；不宣称没有长期泄漏或手机动态性能通过。
- 原6项证据测试通过；D025后补2项阶段语义测试，共8项。378份证据哈希完整，默认`verify.py`按UI设计阶段通过；正式玩法、存档/奖励、平台、美术和安卓工作列入后续开发。细目与复现命令见17及 `readiness.json`。
- D024收到所有者“采用上述统一规则”：呼噜基础公式按PRODUCT，危险预警75%/90%/95%，仅Flowing生成、解析暂停生成/整链移动。Q003已解决，05/GDD/PRODUCT/CORE/TECH/QA已同步；旧运行逻辑仍待TECH落实，不能把规格更新当代码完成。
- 最终包 `build/ui08-flow-verified/`、`build/ui08-catalog-verified/`各47文件；同字节临时构建由本机62320(PID11688)/62330(PID23217)提供，保留供审阅，未开放LAN；用户Creator和旧服务保留。本窗口未提交/推送，期间检测到非本窗口创建的提交`30ead9c`，保留不改写；关闭测试浏览器后没有后台自动开发。
- UI-08设计阶段完成并释放认领。后续代码开发按D024及R01～R08接通百关真实玩法/状态，再完成平台、美术和安卓验收；这些不阻断已完成的UI阶段，不另造第三套预览场景。
...
**D025明确UI-08设计阶段已完成，D024确认Q003三项规则。** 交付见 [17](17_UI_RELEASE_READINESS.md)，正式玩法、平台、美术及安卓验收放在后续代码开发阶段。D021确认UI-05验收，D019首页布局继续有效；UI-06流程见 [15](15_UI_FLOW_PREVIEW.md)，UI-07静态百关见 [16](16_UI_CATALOG_QA.md)。

UI-05按所有者反馈验收通过，Q002在已交付试样范围解决；UI-06新页仍需手机复验。UI-R03通过三尺寸浏览器44 CSS px热区及七关包络检查；UI-Q01小尺寸细节、UI-Q02动态辨识无独立测量证据。Q003由D024解决，Q004由D019解决；下方历史未决记录不再构成重复询问依据。
